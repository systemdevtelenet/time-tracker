...
export default function RosterTable({
  ...
  return (
    <div className="animate-in fade-in">
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-[#434142] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-slate-50/50 dark:bg-[#272626]">
        
        {/* Title & Count - Bold */}
        <div className="flex items-center gap-2">
          <LayoutGrid className="w-4 h-4 text-[#2F6798] dark:text-[#3678B0]" />
          <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-[#F8F8F6] uppercase tracking-tight font-sans">
        ...
        <div className="flex items-center gap-3 text-xs font-semibold text-slate-600 dark:text-slate-400 flex-wrap self-stretch sm:self-auto justify-end">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <select
              ...
              className="px-2 py-1 rounded-lg bg-white dark:bg-[#363435] border border-slate-200 dark:border-[#434142] text-xs font-bold text-slate-800 dark:text-[#F8F8F6] outline-none cursor-pointer"
            >
          ...
          <button
            ...
            className="p-2 rounded-xl bg-white dark:bg-[#363435] hover:bg-slate-100 dark:hover:bg-[#434142] border border-slate-200 dark:border-[#434142] text-slate-600 dark:text-slate-300 transition-colors cursor-pointer shadow-2xs"
          >
            <RotateCw className={`w-3.5 h-3.5 text-[#2F6798] dark:text-[#3678B0] ${isRefreshing ? 'animate-spin' : ''}`} />
          </button>
          ...
          <button
            type="button"
            onClick={exportCSV}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#2F6798] hover:bg-[#235179] active:bg-[#1c4366] dark:bg-[#3678B0] dark:hover:bg-[#2F6798] text-white text-xs font-black shadow-sm transition-all cursor-pointer"
          >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs border-collapse font-sans">
          ...
          {/* Table Header */}
          <thead>
            <tr className="bg-slate-50/90 dark:bg-[#1D2433] text-slate-500 dark:text-slate-300 font-semibold border-b border-slate-200/80 dark:border-[#434142] uppercase text-[10px] tracking-wider select-none">
              
              <th 
                onClick={() => handleSort('name')}
                className="py-2.5 px-4 font-semibold cursor-pointer hover:text-[#2F6798] dark:hover:text-[#3678B0] transition-colors whitespace-nowrap"
              >
                <div className="flex items-center gap-1">
                  <span>EMPLOYEE NAME</span>
                  <ArrowUpDown className="w-3 h-3 text-[#2F6798] dark:text-[#3678B0] opacity-70" />
                </div>
              </th>

              <th 
                onClick={() => handleSort('employeeCode')}
                className="py-2.5 px-3 font-semibold cursor-pointer hover:text-[#2F6798] dark:hover:text-[#3678B0] transition-colors whitespace-nowrap"
              >
                <div className="flex items-center gap-1">
                  <span>ID</span>
                  <ArrowUpDown className="w-3 h-3 text-[#2F6798] dark:text-[#3678B0] opacity-70" />
          ...
          {/* Table Body */}
          <tbody className="divide-y divide-slate-100 dark:divide-[#434142] font-medium">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full bg-[#2F6798]/10 text-[#2F6798] dark:bg-[#1D2433] dark:text-[#F8F8F6] font-bold text-[10.5px] flex items-center justify-center shrink-0 border border-[#2F6798]/20 dark:border-[#434142]">
                          {initials}
                        </div>
                        <span className="font-semibold text-[#2F6798] dark:text-[#3678B0] hover:text-[#1d4b72] dark:hover:text-blue-300 transition-colors">
                          {emp.name}
                        </span>
                      </div>
                    </td>
                    ...
                    {/* ID */}
                    <td className="py-2.5 px-3 font-semibold text-slate-700 dark:text-[#F8F8F6] whitespace-nowrap">
                      {emp.employeeCode}
                    </td>
                    ...
                    <td className="py-2.5 px-3 font-bold text-slate-900 dark:text-[#F8F8F6] bg-slate-50/50 dark:bg-[#272626]/50 whitespace-nowrap">
                    ...
                    <td className="py-2.5 px-4 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-1.5">
                        <button
                          ...
                          className="p-1 rounded-md text-[#2F6798] dark:text-blue-300 hover:bg-[#2F6798]/10 dark:hover:bg-blue-900/30 transition-colors cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          title="End Employee Shift"
                          className="p-1 rounded-md text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors cursor-pointer"
                        >
      ...
      <div className="p-3.5 sm:p-4 border-t border-slate-100 dark:border-[#434142] bg-white dark:bg-[#363435] text-xs flex flex-col sm:flex-row items-center justify-between gap-3 select-none font-sans">
        <div className="text-slate-500 dark:text-slate-400 font-medium">
          Showing <span className="font-bold text-slate-800 dark:text-[#F8F8F6]">{filteredEmployees.length === 0 ? 0 : (currentPage - 1) * rowsPerPage + 1}</span> to <span className="font-bold text-slate-800 dark:text-[#F8F8F6]">{Math.min(currentPage * rowsPerPage, filteredEmployees.length)}</span> of <span className="font-bold text-slate-800 dark:text-[#F8F8F6]">{filteredEmployees.length}</span> employees
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-xs font-semibold flex items-center gap-1 transition-all ${
              currentPage <= 1
                ? 'opacity-40 cursor-not-allowed border-slate-200 dark:border-[#434142] text-slate-400'
                : 'cursor-pointer hover:bg-slate-50 dark:hover:bg-[#272626] border-slate-200 dark:border-[#434142] text-slate-700 dark:text-[#F8F8F6] shadow-2xs'
            }`}
          >
            ...
            <button
              ...
              className={`w-8 h-8 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center ${
                currentPage === pageNum
                  ...
                  : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-[#272626] border border-slate-200/80 dark:border-[#434142]'
              }`}
            >
              {pageNum}
            </button>
