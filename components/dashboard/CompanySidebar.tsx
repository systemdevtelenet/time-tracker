...
export default function CompanySidebar({
  ...
  // Trainers, Trainees, QA, and Agents only see Dashboard, Time Tracking, Flow Hub, Activity Logs
  // Heads of Training, Heads of Quality/QA Supervisors, and Admins have full access including Attendance & Roster and Analytics
  const visibleNavItems = isHeadOrAdmin 
    ? NAV_ITEMS 
    : NAV_ITEMS.filter((item) => item.id !== 'analytics' && item.id !== 'attendance');
  ...
  return (
    <>
      ...
      <aside 
        className={`shrink-0 bg-[#2F6798] dark:bg-[#201F20] dark:border-r dark:border-[#434142] text-white flex flex-col justify-between transition-[width] duration-200 ease-out select-none z-50 shadow-xl ${
          isCollapsed ? 'w-20' : 'w-64'
        } h-screen sticky top-0 overflow-hidden relative`}
      >
        {/* Decorative Bottom Artwork Background */}
        <div 
        ...
        <div className="relative z-10 flex-1 flex flex-col min-h-0">
          ...
          <div className={`p-4 border-b border-[#2F6798]/20 dark:border-[#434142] flex items-center ${
            ...
            {!isCollapsed ? (
              <>
                <div className="flex items-center gap-2.5 overflow-hidden">
                  ...
                  <div className="truncate">
                    ...
                    <p className="text-[10px] font-medium tracking-[0.05em] uppercase text-white/60 dark:text-[#94A3B8] mt-1">
                      Workforce Portal
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="p-2 rounded-xl hover:bg-white/10 dark:hover:bg-[#363435] text-white/80 hover:text-white transition-colors cursor-pointer shrink-0"
              ...
              <button
                ...
                className="p-2 rounded-xl hover:bg-white/10 dark:hover:bg-[#363435] text-white/80 hover:text-white transition-colors cursor-pointer mx-auto"
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                    isActive
                      ? 'bg-white/20 dark:bg-[#363435] dark:border dark:border-[#3678B0]/60 font-bold text-white shadow-xs'
                      : 'text-white/75 dark:text-[#94A3B8] hover:bg-white/10 dark:hover:bg-[#2C2A2B] hover:text-white dark:hover:text-[#F8F8F6]'
                  } ${isCollapsed ? 'justify-center px-2' : ''}`}
                  title={item.label}
                >
                  <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-white' : 'text-white/75 dark:text-[#94A3B8]'}`} />
        ...
        {/* Sidebar Footer (Punch Status, Settings & User Profile Card) */}
        <div className="p-3 border-t border-[#2F6798]/20 dark:border-[#434142] relative z-10 space-y-2 shrink-0">
          
          {/* Compact Punch Clock Action Card */}
          {!isCollapsed ? (
            <div className="p-2.5 rounded-2xl bg-white/10 dark:bg-[#363435] backdrop-blur-md border border-white/15 dark:border-[#434142] space-y-1.5 shadow-xs">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-white/80 dark:text-[#F8F8F6]/90 uppercase tracking-wider flex items-center gap-1">
              ...
              {currentStatus === 'lunch' ? (
                <button
                  ...
                  className="w-full py-1.5 px-2.5 rounded-lg bg-[#C8A54B] hover:bg-[#b8953d] active:bg-[#a68430] text-slate-950 font-extrabold text-xs shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                >
            ...
            <div className="flex justify-center">
              {currentStatus === 'lunch' ? (
                <button
                  ...
                  disabled={isPunching}
                  onClick={() => handleActionClick('End Lunch')}
                  className="p-2.5 rounded-xl bg-[#C8A54B] text-slate-950 shadow-xs cursor-pointer disabled:opacity-50"
                  title="End Lunch"
                >
          ...
          <button
            ...
            title="Settings"
          >
            <Settings className={`h-4 w-4 shrink-0 ${currentTab === 'settings' ? 'text-white' : 'text-white/75 dark:text-[#94A3B8]'}`} />
            ...
            <div className="p-2.5 rounded-2xl bg-white/10 dark:bg-[#363435] border border-white/10 dark:border-[#434142] flex items-center justify-between gap-2 shadow-xs">
              <div className="flex items-center gap-2.5 overflow-hidden">
                ...
                <div className="truncate">
                  ...
                  <div className="text-[10px] font-medium text-white/60 dark:text-[#94A3B8] tracking-wider uppercase truncate leading-tight mt-0.5">
                    {supervisor.position || 'EMPLOYEE'}
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                title="Log Out"
                className="p-1.5 rounded-xl hover:text-white hover:bg-[#ED1C25]/20 active:bg-[#ED1C25]/30 text-white/75 dark:text-[#94A3B8] transition-colors cursor-pointer shrink-0"
              >
